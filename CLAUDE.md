...
#### CI - Documentation (`ci.yml`)
Автоматические проверки при push/PR:
- ✅ Структура документации
- ✅ Синтаксис bash скриптов
- ✅ Naming conventions
- ✅ Отсутствие недопустимых файлов

#### Gemini AI Review (`gemini-docs-review.yml`)
Интеллектуальное ревью документации при PR:
...
**Документация:**
- [.github/README.md](.github/README.md) - обзор workflows
- [.github/GEMINI_SETUP.md](.github/GEMINI_SETUP.md) - настройка Gemini review

### Локальная проверка перед коммитом

```bash
# Проверка bash скриптов
bash -n .claude/scripts/r2r
bash -n .claude/scripts/commands/*.sh

# Проверка структуры документации
test -d docs/r2r && test -d docs/fastmcp && test -d docs/claude_code

# Проверка naming conventions
fd -e md . docs/r2r/ | grep -E '0[0-9]-.*\.md|README\.md'
```

## 🚫 Запрещенные действия

1. **НЕ создавай** package.json, pyproject.toml, requirements.txt - нет кодовой базы
2. **НЕ меняй** язык документации на английский без явного запроса
3. **НЕ удаляй** эмодзи из заголовков - это часть стиля
4. **НЕ создавай** .cursorrules, AGENTS.md и подобные файлы - используй только CLAUDE.md
5. **НЕ используй** grep, find, cat - используй rg, fd, bat (современные альтернативы)
6. **НЕ меняй** CI/CD workflows без необходимости - они настроены под проект
